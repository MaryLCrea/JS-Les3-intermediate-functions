...
function getEmailDomain(mailadres) {
const indexOfApenstaart = mailadres.indexOf('@');
const domain = mailadres.substring(indexOfApenstaart +1);
...
function typeOfEmail(mailadresje) {
    const indexOfApenstaartje = mailadresje.indexOf('@');
    const domain = mailadresje.substring(indexOfApenstaartje + 1);
    console.log(domain);
...
function checkEmailValidity(mailadres) {
    const apenstaart = '@';
    const geenKomma = ',';
    const geenLastPunt = '.nl';

    if (mailadres.includes(apenstaart)&& !mailadres.includes(geenKomma)&& mailadres.includes(geenLastPunt)) {
return true;
    } else {
        return false
    }
